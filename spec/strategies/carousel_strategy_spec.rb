...
RSpec.describe GoogleCarouselParser::CarouselStrategy do
  ...
  describe '.extract' do
    ...
    it 'extracts item with all fields' do
      ...
      expect(first_item[:name]).to eq('The Starry Night')
      expect(first_item[:extensions]).to eq(['1889'])
      expect(first_item[:link]).to start_with('https://www.google.com')
      expect(first_item[:image]).to start_with('data:image/jpeg;base64')
    end

    it 'extracts items with empty extensions when date is missing' do
      sunflowers_item = items.find { |item| item[:name] == 'Sunflowers' }
      expect(sunflowers_item[:extensions]).to eq([])
    end
  ...
  describe '.strategy_name' do
    it 'returns carousel' do
      expect(described_class.strategy_name).to eq('carousel')
    end
  end
end
