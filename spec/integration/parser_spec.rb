...
RSpec.describe GoogleCarouselParser::Parser do
  shared_examples 'parses carousel correctly' do |html_file, expected_file, expected_count, first_item_name, first_item_date|
    ...
    describe '.parse_file' do
      ...
      it 'extracts correct number of items' do
        expect(items.size).to eq(expected_count)
      ...
      it 'matches first item completely' do
        expect(actual_data.first[:name]).to eq(first_item_name)
        expect(actual_data.first[:extensions]).to eq([first_item_date])
  ...
  context 'with Van Gogh paintings' do
    include_examples 'parses carousel correctly',
  ...
  context 'with Da Vinci paintings' do
    include_examples 'parses carousel correctly',
      'spec/fixtures/da-vinci-paintings.html',
      'spec/fixtures/da-vinci-expected-array.json',
      46,
      'Salvator Mundi',
      '1500'
  end

  shared_examples 'parses grid correctly' do |html_file, expected_file, expected_count, first_item_name, first_item_date|
    let(:expected_data) do
      JSON.parse(File.read(expected_file), symbolize_names: true)[:films_produced]
    end
    ...
    describe '.parse_file' do
      ...
      it 'uses grid strategy' do
        expect(GoogleCarouselParser::GridStrategy).to receive(:applicable?).and_call_original
        described_class.parse_file(html_file)
      end
    end
  end

  context 'with Marvel movies (grid)' do
    include_examples 'parses grid correctly',
      'spec/fixtures/marvel-movies.html',
      'spec/fixtures/marvel-movies-expected-array.json',
      51,
      'Iron Man',
      '2008'
  end
